...
export class MatSelectCaseInitializer {
  matSelectSearchService!: MatSelectCaseService;
  ...
  init(): () => Promise<boolean> {
    return () => {
      ...
      return new Promise<any>((resolve, reject) => {
        if (
          window.screen.width <= (this.configs.maxWidthForMobileView as any)
        ) {
        ...
        this._handleClasses(
          this.document.documentElement.clientWidth <=
            (this.configs.maxWidthForMobileView as any)
        );
        {
          this._handleClasses(this.mobileQuery.matches);
        }
