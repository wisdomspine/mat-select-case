...
import { MatSelectCaseMediaTracker } from './mat-select-case-media-tracker';
...
@Directive({
  selector: 'mat-select[matSelectCase]',
  exportAs: 'matSelectCase',
})
export class MatSelectCaseDirective
  extends MatSelectCaseMediaTracker
  implements OnInit, AfterViewInit, OnDestroy, OnChanges
{
  ...
  @Input() maximumResultForShow?: number;
  @Input() useMobileView?: boolean | null;
  @Input()
  mobileViewType?: 'FullScreen' | 'BottomSheet';
  @Input()
  valueMember?: string;
  @Input()
  ...
  loadingContainer = `<div class="mat-select-case-loading-container"><div class="loadingio-spinner-ellipsis-ao6bq46b8nt"><div class="ldio-it58uqzfg5">
  ...
  ngAfterViewInit(): void {
    ...
    this.matSelect.compareWith = (o1, o2): boolean => {
      if (o1 && o2) {
        if (typeof o1 === 'object') {
          return o1[this.valueMember as any] === o2[this.valueMember as any];
        } else {
          return o1 === o2;
        }
      }
    ...
    if (
      ...
      this.searchValue = this.inFirstLoadSearcherValue as any;
      try {
        this._onSearch()
          .then((res) => {
            this.source = res;
  ...
  private _bindAfterClick(): void {
    ...
    const parentEl =
      this._currentEl.parentElement?.parentElement?.parentElement
        ?.parentElement;
  ...
  private _addLoadingElementForScrollInf(): void {
    ...
    container.classList.add('mat-select-case-scroll-loading');
  ...
  private _addMainLoading(): void {
    ...
    el.classList.add('mat-select-case-main-loading');
  ...
  scrollInfChange(event: Event): void {
    ...
    if (el) {
      ...
      if (
        ...
        if (checkSourceLen < (this.maximumResultForShow as any)) {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              this._onSearch(true).then((items) => {
                ...
                this._hasMoreRow = len >= (this.maximumResultForShow as any);
                ...
                this._syncSourceAndOptions(
                  ...
                  this._pageNumber * (this.maximumResultForShow as any),
                  pagedSrc
                );
                if (!this._hasMoreRow) {
  ...
  private _getListMainContainerElement(): HTMLElement {
    ...
    if (this.mobileQuery.matches && this.useMobileView) {
      el = this._listBoxEl?.parentElement as any;
    } else {
      el = this._listBoxEl as any;
    }
    return el;
  }
  ...
  private _addNoMoreResultElement(): void {
    if (this._listBoxEl) {
      const container = document.createElement('div');
      container.classList.add('mat-select-case-scroll-no-more');
      container.innerText = this.noMoreResultLabel as any;
  ...
  private _moveCursorToEnd(el: any) {
  ...
  private _addSearchablePanelClass(parent: HTMLElement): void {
    if (this._listBoxEl) {
      if (!parent.classList.contains('mat-select-case-panel')) {
        parent.classList.add('mat-select-case-panel');
      }
    }
  }
  ...
  private _createHeaderElement(): HTMLElement {
    ...
    <input type='text'
     ...
     class="mat-select-case-search-box-input" value="${this.searchValue}" >
    <i class="mat-select-case-search-box-icon"></i>
    ...
    inputContainerEl.classList.add('mat-select-case-search-box-wrap');
    inputContainerEl.appendChild(searchElement);
    searchElement.classList.add('mat-select-case-search-box-container');
    ...
    if (this._listBoxEl) {
      ...
      setTimeout(() => {
        if (this.hasSearchBox) {
          ...
          this._searchBoxInputEl = inputEl as any;
          if (!this.mobileQuery.matches || !this.useMobileView) {
            inputEl?.focus();
            this._moveCursorToEnd(inputEl);
          }
          this._handleObserverForInputValue(inputEl as any);
        }
      });
    }
  }
  ...
  private _checkKeyCode(event: any) {
  ...
  private _hideOptions(): void {
    this.filteredSource
      ...
      .forEach((hide) => {
        ...
        const option = (ops as any).find((z: any) =>
          this._getExpForOption(hide, z.value)
        );
        if (option) {
  ...
  private _getPagedResult(src: any[]): any[] {
    let result = [];
    const startIndex =
      (this.maximumResultForShow as any) * (this._pageNumber - 1);
    let endIndex = (this.maximumResultForShow as any) * this._pageNumber;
  ...
  private _onSearch(changePage = false): Promise<any[]> {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      try {
        this._addSearchablePanelClass(this._listBoxEl as any);
        if (!this.lazyLoad) {
          let filteredSource = [];
          if (this.isStringArray) {
            ...
            filteredSource = this.source.filter((x) =>
              this._getRecursiveValues(x).some((value: any) =>
                regexp.test(`${value ?? ''}`)
              )
            );
          }
          ...
          if (this.searcher && typeof this.searcher === 'function') {
            this.searcher(
              this.searchValue,
              this._pageNumber,
              this.maximumResultForShow as number
            )
              .toPromise()
              .then((res) => {
                if (!changePage) {
  ...
  private _appendNotFoundHtmlTag(): void {
    const el = document.createElement('div');
    el.classList.add('mat-select-case-not-found-container');
    el.innerHTML = this.emptyLabel as any;
    this._listBoxEl?.appendChild(el);
  }
  ...
  private _handleNotFoundLabel(filteredSource: any[]): boolean {
    ...
    if (this._listBoxEl) {
      const el = this._listBoxEl.querySelector(
        '.mat-select-case-not-found-container'
      ) as HTMLElement;
      if (el) {
        if (show) {
  ...
  private _getExp(left: any, right: any): any {
    if (right[this.valueMember as any]) {
      return left[this.valueMember as any] == right[this.valueMember as any];
    } else {
      return left[this.valueMember as any] == right;
    }
  }

  private _getExpForOption(left: any, right: any): any {
    if (left[this.valueMember as any]) {
      if (right[this.valueMember as any]) {
        return left[this.valueMember as any] == right[this.valueMember as any];
      } else {
        return left[this.valueMember as any] == right;
  ...
  private _changeMedia(): void {
    ...
    if (!classes.includes('mat-select-case-opacity')) {
      classes.push('mat-select-case-opacity');
    }
    if (this.mobileQuery.matches && this.useMobileView) {
      const hasMyClass = classes.includes('mat-select-case-mobile');
      if (this.matSelect) {
        if (!hasMyClass) {
          classes.push('mat-select-case-mobile');
        }
      }
    } else {
      classes = classes.filter((z) => z !== 'mat-select-case-mobile');
    }
  ...
  private _addTitleForMobileView(spacer: HTMLElement): void {
    if (spacer && !this.hasSearchBox) {
      ...
      titleEl.classList.add('mat-select-case-mobile-title');
  ...
  private _removeOpacity(): void {
    setTimeout(() => {
      this._listBoxEl?.classList.remove('mat-select-case-opacity');
    });
  }
  ...
  private _afterOpenForMobile(spacer: HTMLElement): void {
    ...
    if (this.mobileQuery.matches) {
      ...
      this._toggleParentClass(
        panel,
        'mat-select-case-mobile-wrapper',
        '.cdk-overlay-pane'
      );
      if (this.mobileViewType === 'BottomSheet') {
        ...
        this._toggleParentClass(
          panel,
          'mat-select-case-mobile-open-bottomToTop',
          '.cdk-overlay-pane'
        );
      } else {
        this._toggleParentClass(
          panel,
          'mat-select-case-mobile-open-fullScreen',
          '.cdk-overlay-pane'
        );
      }
  ...
  private _getRecursiveValues(object: any): string[] {
    const values = [];
